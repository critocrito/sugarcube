...
const html = n => n
  .end()
  .evaluate(() => document.body.innerHTML); // eslint-disable-line no-undef
...
export const search = curry((url, headless, term) => flow([
  browse(headless),
  n => n
    .type('#lst-ib', term)
    .click('#_fZl')
...
export const images = curry((url, scrollCount, headless, term) => flow([
...
export const reverseImagesFromFile = curry((url, headless, path) => flow([
  browse(headless),
  n => n
    .click('a.gsst_a')
    .wait('#qbf a')
